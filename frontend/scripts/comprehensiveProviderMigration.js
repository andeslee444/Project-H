...
function parseInsuranceData() {
  ...
  insuranceData.forEach(row => {
    ...
    // Always add Self-Pay as an option
...
function transformProviderForSupabase(provider, index, insuranceMap) {
  // Generate UUID
  ...
  return {
    ...
    email: `${provider.first_name.toLowerCase()}.${provider.last_name.toLowerCase().replace(/\s+/g, '')}@mindcare.com`,
...
async function migrateProviders() {
  console.log('Starting comprehensive provider migration...\n');
  ...
  const transformedProviders = allProviders.map((provider, index) => 
    transformProviderForSupabase(provider, index, insuranceMap)
  );
  ...
  sample?.forEach(p => {
    ...
    console.log(`  Address: ${p.location.address}, ${p.location.city}, ${p.location.state} ${p.location.zip}`);
